...
class _RecipeDetailScreenState extends State<RecipeDetailScreen> {
  ...
  Future<void> _refreshRecipe() async {
    await Future.delayed(const Duration(milliseconds: 500)); // 編集の反映待ち
    await fetchRecipe(widget.recipe.id);
    await fetchUserRating(widget.recipe.id);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
          ...
          if (isMyRecipe())
            IconButton(
              ...
              onPressed: () async {
                ...
                if (updated == true) {
                  print('レシピが更新されました');
                  await _refreshRecipe(); // ← ここで共通関数を呼び出す
                }
              },
            ),
          IconButton(
      ...
      body: RefreshIndicator(
        onRefresh: _refreshRecipe, // ← 追加
        child: SingleChildScrollView(
          physics: const AlwaysScrollableScrollPhysics(), // 必須
          ...
          child: Column(
            ...
            children: [
              ...
              // 作成日
              Text(
                '投稿日: ${DateFormat('yyyy/MM/dd').format(recipe.createdAt)}',
